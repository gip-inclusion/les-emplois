...
def inactive_jobseekers_without_recent_related_objects(inactive_since, batch_size=None, notified=False):
    filter_for_unnotified = not notified
    ...
    qs = (
        User.objects.filter(
            ...
            upcoming_deletion_notified_at__isnull=filter_for_unnotified,
        )
        .filter(
