...
class AnonymizedApplication(models.Model):
    ...
    has_approval = models.BooleanField(verbose_name="a eu un PASS IAE", default=False)
...
class AnonymizedApproval(models.Model):
    ...
    def __str__(self):
        return f"PASS IAE {self.id} anonymisé le {self.anonymized_at.strftime('%Y-%m-%d %H:%M:%S')}"


class AbstractAnonymizedEligibilityDiagnosis(models.Model):
...
class AnonymizedSIAEEligibilityDiagnosis(AbstractAnonymizedEligibilityDiagnosis):
    ...
    def __str__(self):
        return (
            f"diagnostic d'éligibilité SIAE {self.id} anonymisé le {self.anonymized_at.strftime('%Y-%m-%d %H:%M:%S')}"
        )
...
class AnonymizedGEIQEligibilityDiagnosis(AbstractAnonymizedEligibilityDiagnosis):
    ...
    def __str__(self):
        return (
            f"diagnostic d'éligibilité GEIQ {self.id} anonymisé le {self.anonymized_at.strftime('%Y-%m-%d %H:%M:%S')}"
        )
