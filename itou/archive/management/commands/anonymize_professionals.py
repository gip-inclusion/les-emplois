...
class Command(BaseCommand):
    ...
    def anonymize_professionals_after_grace_period(self):
        ...
        users_not_to_delete = [user for user in users if user not in users_to_delete]
        users_to_anonymize = self.get_users_to_anonymize_without_deletion(users_not_to_delete)
    ...
    def get_users(self, grace_period_since):
        return list(
            ...
            .order_by("is_deactivated", "upcoming_deletion_notified_at")[: self.batch_size]
        )
    ...
    def get_users_to_anonymize_without_deletion(self, users):
        ...
        return list(
            User.objects.filter(id__in=[user.id for user in users], email__isnull=False).select_for_update(
    ...
    def anonymize_professionals_without_deletion(self, users):
        for model in [CompanyMembership, InstitutionMembership, PrescriberMembership]:
            model.objects.filter(user_id__in=[user.id for user in users]).update(is_active=False)

        User.objects.filter(id__in=[user.id for user in users]).update(
            is_active=False,
