...
class Command(BaseCommand):
    ...
    def reset_notified_jobseekers_with_recent_activity(self):
        ...
        users_to_reset_qs = User.objects.filter(
            kind=UserKind.JOB_SEEKER, upcoming_deletion_notified_at__isnull=False
        ).exclude(
            id__in=[
                user.id for user in inactive_jobseekers_without_recent_related_objects(inactive_since, notified=True)
            ]
        )
