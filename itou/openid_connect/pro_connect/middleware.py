...
class ProConnectLoginMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if len(auto_login) != 1:
            logger.info("Nexus auto login: Multiple tokens found -> ignore auto_login")
        ...
        if email is None:
            logger.info("ProConnect auto login: Missing email in auto_login token -> ignore auto_login")
            return HttpResponseRedirect(new_url)
