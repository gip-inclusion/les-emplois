...
class ItouUserAdmin(InconsistencyCheckMixin, CreatedOrUpdatedByMixin, ItouModelMixin, UserAdmin):
    ...
    def deactivate_view(self, request, user_pk):
        ...
        now = timezone.now()
        PrescriberMembership.include_inactive.filter(user=user, is_active=True).update(
            is_active=False, updated_by=request.user, updated_at=now
        )
        CompanyMembership.include_inactive.filter(user=user, is_active=True).update(
            is_active=False, updated_by=request.user, updated_at=now
        )
        InstitutionMembership.include_inactive.filter(user=user, is_active=True).update(
            is_active=False, updated_by=request.user, updated_at=now
        )
        ...
        messages.info(request, format_html("Désactivation de l'utilisateur {user} effectué.", user=user))
