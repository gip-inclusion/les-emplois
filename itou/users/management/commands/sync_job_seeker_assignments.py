...
def get_users_contacts(ids):
    job_seekers_qs = (
        ...
        .annotate(
            geiq_diagnosis_authors=ArraySubquery(
                ...
                .filter(author__kind__in=UserKind.caseworkers())
                .values(
                    json=JSONObject(
                        user_id="author_id",
    ...
    for job_seeker in job_seekers_qs:
        ...
        if job_seeker.created_by and job_seeker.created_by.kind in UserKind.caseworkers():
            contacts[
                (
...
class Command(BaseCommand):
    help = "Create objects that link caseworkers to assigned job seekers."
        ...
        for chunks_count, job_seekers_ids in enumerate(batched(job_seekers_pks, CHUNK_SIZE)):
            ...
            with transaction.atomic():
                ...
                for job_seeker_id, contacts in users_contacts.items():
                    for (caseworker_id, prescriber_organization_id, company_id), actions in contacts.items():
                        ...
                        if assignment := assignments.get(
                            (job_seeker_id, caseworker_id, prescriber_organization_id, company_id)
                        ):
                            # update assignment
