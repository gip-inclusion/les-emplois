...
class TransferField(TextChoices):
    ...
    BRAND = "brand", models.Company._meta.get_field("brand").verbose_name.title()
    DESCRIPTION = "description", models.Company._meta.get_field("description").verbose_name.title()
    PHONE = "phone", models.Company._meta.get_field("phone").verbose_name.title()
    ...
    for transfer_field in fields_to_transfer:
        ...
        if model_field := spec.get("model_field"):
            ...
            if from_value != old_to_value:
                ...
                if detailed_report:
                    item = f"{model_field.name}: {old_to_value!r} remplacé par {from_value!r}"
                else:
                    item = f"{model_field.name}: mis à jour"
                reporter.add(transfer_field, item)
        else:
