...
from itou.companies.admin_forms import CompanyAdminForm, SelectTargetCompanyForm
from itou.users.admin_forms import ChooseFieldsToTransfer
...
class CompanyAdmin(ItouGISMixin, OrganizationAdmin):
    ...
    def transfer_view(self, request, from_company_pk, to_company_pk=None):
        # TODO: ajout de warning pour les entreprises ayant comme source l'export ASP
        # TODO: ajout de warning pour les entreprises participant à un contrôle a posteriori

        if not self.has_change_permission(request):
            raise PermissionDenied

        from_company = get_object_or_404(
            models.Company.objects.exclude(source=models.Company.SOURCE_ASP), pk=from_company_pk
        )
        ...
        if not to_company:
            ...
            for transfer_field in transfer.TransferField:
                ...
                if model_field := spec.get("model_field"):
                    if transfer_data[transfer_field]["data"] == [getattr(to_company, model_field.name)]:
                        transfer_data[transfer_field]["data"] = [
                            "Rien à transférer: valeur identique pour les deux entreprises"
                        ]

                if from_data := transfer_data[transfer_field]["data"]:
                ...
                if to_constraint := spec.get("to_constraint"):
                    # TODO: add warning explaining the problem
                    print(to_constraint)
                    pass
            ...
            if request.POST and form.is_valid():
                reporter = transfer.transfer_company_data(
                    from_company, to_company, form.cleaned_data["fields_to_transfer"]
                )
                summary_lines = [
                    "-" * 20,
        ...
        context = self.admin_site.each_context(request) | {
            ...
            "from_company": from_company,
            "to_company": to_company,
            "nothing_to_transfer": False,  # not fields_choices,
