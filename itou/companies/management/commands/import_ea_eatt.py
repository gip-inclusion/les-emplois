...
class Command(BaseCommand):
    ...
    def process_file(self, file, *, wet_run=False):
        ...
        self.logger.info("Rows used with a SIRET", extra={"count": info_stats["rows_with_a_siret"]})
        self.logger.info("Rows used with an empty email", extra={"count": info_stats["rows_with_empty_email"]})
        self.logger.info("Rows creatable", extra={"count": info_stats["creatable_sirets"]})
        self.logger.info("Rows created", extra={"count": info_stats["structures_created"]})
        if info_stats["not_created_because_of_missing_email"]:
            self.logger.info(
                "Rows not created because of missing email",
                extra={"count": info_stats["not_created_because_of_missing_email"]},
            )
        self.logger.info("Rows updatable", extra={"count": info_stats["updatable_sirets"]})
        self.logger.info("Rows updated", extra={"count": info_stats["structures_updated"]})
        self.logger.info("Rows deletable", extra={"count": info_stats["deletable_sirets"]})
        self.logger.info("Rows deleted", extra={"count": info_stats["structures_deleted"]})
        self.logger.info("Rows undeletable", extra={"count": info_stats["structures_undeletable"]})
        self.logger.info("Rows deletable skipped", extra={"count": info_stats["structures_deletable_skipped"]})
