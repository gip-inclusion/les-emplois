...
@require_POST
def create_process_orientation_link_view(request, orientation_id):
    if not request.user.has_perm(f"{Orientation._meta.app_label}.create_process_orientation_link_view"):
...
class OrientationAdmin(InsertionAdmin):
    ...
    ordering = ["-created_at"]
    inlines = [OrientationTransitionLogInline, ProcessOrientationLinkInline]
    fieldsets = [
        (
            "Orientation",
            {
    ...
    def get_urls(self):
        ...
        return [
            path(
                ...
                self.admin_site.admin_view(create_process_orientation_link_view),
