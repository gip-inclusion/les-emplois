...
class Orientation(xwf_models.WorkflowEnabled, models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=(Q(status=OrientationStatus.REFUSED) & ~Q(refusal_reasons=[]) & ~Q(refusal_details=""))
                | (~Q(status=OrientationStatus.REFUSED) & Q(refusal_reasons=[], refusal_details="")),
    ...
    def notification_expired_for_beneficiary(self):
        return orientation_notifications.ExpiredOrientationForBeneficiary(self.beneficiary, orientation=self)

    @property
    def notification_expired_for_sender(self):
        return orientation_notifications.ExpiredOrientationForSender(self.sender, orientation=self)

    # Emails (to users that do not have an account)
    @property
    def email_new_orientation_for_referent(self):
    ...
    @property
    def email_accepted_orientation_for_referent(self):
        to = [self.referent_email]
        context = {
            "orientation": self,
        }
    ...
    @property
    def email_expired_orientation_for_referent(self):
        to = [self.referent_email]
        context = {
            "orientation": self,
        }
...
class OrientationProcessLink(models.Model):
    ...
    def process_link(self):
        ...
        return get_absolute_url(
            reverse("insertion_views:orientation_details_for_service_provider", query={"token": self.pk})
        )
    ...
    # Emails
    @property
    def email_new_orientation_for_structure(self):
    ...
    @property
    def email_accepted_orientation_for_structure(self):
        to = [self.orientation.service.contact_email]
        context = {
            "process_link": self.process_link,
            "orientation": self.orientation,
    ...
    @property
    def email_expired_orientation_for_structure(self):
        to = [self.orientation.service.contact_email]
        context = {
            "process_link": self.process_link,
            "orientation": self.orientation,
    ...
    @property
    def email_new_link_for_structure(self):
        to = [self.orientation.service.contact_email]
        context = {
            "process_link": self.process_link,
            "orientation": self.orientation,
