...
class Command(BaseCommand):
    ...
    def import_structures(self, client):
        ...
        differ = diff.CollectionDiffer(
            ...
            DataInclusionApiItemsIterator(client.structures),
            (["uid"], ["id"]),
    ...
    def _fill_service_from_dora_api_data(self, service, dora_services, disabled_dora_form_di_structures):
        if service.source.value == SOURCE_DORA_VALUE:
            ...
            service.access_conditions_dora = []
            service.mobilization_modes_beneficiaries_external_form_link = ""
            service.mobilization_modes_beneficiaries_external_form_link_text = ""
            service.mobilization_modes_beneficiaries_other = ""
            service.mobilization_modes_professionals_external_form_link = ""
            service.mobilization_modes_professionals_external_form_link_text = ""
            service.mobilization_modes_professionals_other = ""
            service.credentials = []
            service.credentials_documents = []
            service.credentials_online_form = ""
            service.opening_hours_text = ""
            service.contact_is_public = True
    ...
    def _fill_service_related_fields_from_data(self, service, data, dora_services, is_creation):
        ...
        if service.source.value == SOURCE_DORA_VALUE:
            ...
            service.mobilization_modes_beneficiaries.clear()
    ...
    def _fill_and_save_service_from_api_data(
        ...
        self._fill_service_from_dora_api_data(service, dora_services, disabled_dora_form_di_structures)
    ...
    def import_services(self, di_client, dora_client):
        ...
        differ = diff.CollectionDiffer(
            Service.objects.all(),
            DataInclusionApiItemsIterator(di_client.services),
            (["uid"], ["id"]),
            watched_data={"updated_on": "date_maj"},
            comparative_data_converters={"date_maj": datetime.date.fromisoformat},
        )
    ...
    def import_disabled_structures(self, client):
        self.logger.info("Import disabled structures from DORA")

        # Some data·inclusion services are non orientable with DORA's form
        disabled_dora_form_di_structures = client.disabled_dora_form_di_structures()
        updated_services = Service.objects.filter(
            structure__uid__in=disabled_dora_form_di_structures,
        ...
        self.logger.info(
            ...
            updated_services,
            len(disabled_dora_form_di_structures),
        )
    ...
    def handle(self, *args, data, **options):
        with (
            ...
            if ArgumentData.STRUCTURES in data:
                self.import_structures(di_client)
                self.import_disabled_structures(dora_client)

            if ArgumentData.SERVICES in data:
                self.import_services(di_client, dora_client)
