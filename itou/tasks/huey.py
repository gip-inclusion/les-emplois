...
from huey.consumer import BaseProcess, Consumer, Scheduler, Worker
...
class DBStorage(BaseStorage):
    ...
    def add_to_schedule(self, data, ts):
        if is_naive(ts):
            warnings.warn("Received naive datetime, interpreting in the current timezone.")
            ts = make_aware(ts)
    ...
    @staticmethod
    def _encode_key(key):
        # Huey is a bit messy in its handling of bytes vs str for KV.
        # Multiple Huey backends encode keys (RedisExpireStorage, FileStorage).
    ...
    def put_data(self, key, value, is_result=False):
        self.KV.objects.bulk_create(
            [self.KV(key=self._encode_key(key), value=value, is_result=is_result)],
    ...
    def peek_data(self, key):
        try:
            return self.KV.objects.get(key=self._encode_key(key)).value
    ...
    def pop_data(self, key):
        with transaction.atomic():
            try:
                kv = (
                    self.KV.objects.select_for_update(of={"self"}, skip_locked=True)
                    .filter(key=self._encode_key(key))
                    .get()
                )
            except self.KV.DoesNotExist:
                return EmptyData
    ...
    def delete_data(self, key):
        num_delete, _delete_details = self.KV.objects.filter(key=self._encode_key(key)).delete()
        return bool(num_delete)

    def has_data_for_key(self, key):
        return self.KV.objects.filter(key=self._encode_key(key)).exists()

    def put_if_empty(self, key, value):
        with transaction.atomic():
            try:
                with maybe_unique_violation(self.KV, "tasks_kv_key_uniq"):
                    self.KV.objects.create(key=self._encode_key(key), value=value)
                    return True
...
class DBShutdown(BaseProcess):
    def shutdown(self):
        super().shutdown()
        connections.close_all()


class DBWorker(DBShutdown, Worker):
    pass


class DBScheduler(DBShutdown, Scheduler):
    pass


class DBConsumer(Consumer):
    worker_class = DBWorker
    scheduler_class = DBScheduler
