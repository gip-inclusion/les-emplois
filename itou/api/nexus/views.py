...
from itou.api.nexus.serializers import DeleteObjectSerializer, StructureSerializer, SyncSerializer, UserSerializer
from itou.nexus.enums import USER_KIND_MAPPING
from itou.nexus.models import APIFullSync, NexusMembership, NexusStructure, NexusUser
...
class UsersView(NexusApiMixin, generics.GenericAPIView):
    ...
    def perform_create(self, serializer):
        ...
        for user_data in validated_data:
            user_id = unique_id(user_data["source_id"], self.source)
            memberships_data = user_data.pop("memberships")
            for membership_data in memberships_data:
                structure_id = unique_id(membership_data["structure_id"], self.source)
                memberships.append(
                    NexusMembership(
    ...
    def perform_delete(self, serializer):
        NexusUser.objects.filter(source=self.source, **serializer.validated_data).delete()


@extend_schema(exclude=True)
class StructuresView(NexusApiMixin, generics.GenericAPIView):
    def post(self, request, *args, **kwargs):
        serializer = StructureSerializer(data=request.data, many=True, source=self.source)
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)
        return Response({}, status=status.HTTP_202_ACCEPTED, headers={})
    ...
    def perform_create(self, serializer):
        ...
        for structure_data in validated_data:
            structures.append(
                NexusStructure(
                    source=self.source,
                    id=unique_id(structure_data["source_id"], self.source),
                    kind=structure_data["source_kind"],  # No mapping for now
                    **structure_data,
                )
            )
    ...
    def perform_delete(self, serializer):
        NexusStructure.objects.filter(source=self.source, **serializer.validated_data).delete()


@extend_schema(exclude=True)
class SyncView(NexusApiMixin, generics.GenericAPIView):
    def post(self, request, *args, **kwargs):
        serializer = SyncSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        if "start" in serializer.validated_data:
            start_at = self.init_sync()
            return Response({"started_at": start_at.isoformat()}, status=status.HTTP_202_ACCEPTED, headers={})

        if started_at := serializer.validated_data.get("started_at"):
            if APIFullSync.objects.filter(service=self.source, new_start_at=started_at).update(
                new_start_at=None, timestamp=started_at
            ):
                return Response({}, status=status.HTTP_202_ACCEPTED, headers={})
            else:
                logger.warning("Got invalid start_at for source=%s", self.source, exc_info=True)

        return Response({}, status=status.HTTP_403_FORBIDDEN, headers={})

    def init_sync(self):
        now = timezone.now()
        APIFullSync.objects.update_or_create(
            service=self.source,
