...
class UsersView(NexusApiMixin, generics.GenericAPIView):
    ...
    def perform_create(self, serializer):
        ...
        for user_data in validated_data:
            user_id = service_id(user_data["source_id"], self.source)
            memberships_data = user_data.pop("memberships")
            for membership_data in memberships_data:
                structure_id = service_id(membership_data["structure_id"], self.source)
                memberships.append(
                    NexusMembership(
    ...
    def perform_create(self, serializer):
        ...
        for structure_data in validated_data:
            structures.append(
                NexusStructure(
                    source=self.source,
                    id=service_id(structure_data["source_id"], self.source),
                    kind=structure_data["source_kind"],  # FIXME: use d-i data scheme for the mapping
                    **structure_data,
                )
            )
...
class SyncView(NexusApiMixin, generics.GenericAPIView):
    def post(self, request, *args, **kwargs):
        ...
        if "start" in serializer.validated_data:
            ...
            return Response({"started_at": start_at.isoformat()}, status=status.HTTP_200_OK)
