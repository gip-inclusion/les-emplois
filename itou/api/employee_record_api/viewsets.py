...
# Note sur le champ `salarieLangueFrancaise`

Historiquement, le champ `salarieLangueFrancaise` avait la
signification suivante : la valeur `true` indiquait que le salarié
**maîtrise** la langue.

La signification de ce champ dans l'ASP s'est inversée au 09/07/2026 : la
valeur `true` indique(ra) que le salarié ne maîtrise **PAS** la langue
(afin de dénoter un frein à l'inclusion, comme les autres champs
similaires : `salarieSortantASE`, `salarieParentIsole`,
`salarieHebergement`, etc.).

Afin d'être homogène avec l'ASP, l'API des Emplois évolue pour
permettre d'avoir la même signification pour ce champ. Pour cela, le
client doit envoyer l'en-tête HTTP
`X-API-salarieLangueFrancaise-like-asp`. La simple présence de
l'en-tête active le comportement, quelle que soit la valeur fournie.

En résumé :

1. **Avec** l'en-tête HTTP `X-API-salarieLangueFrancaise-like-asp`,
   une valeur `true` pour ce champ signifie que le salarié ne maîtrise
