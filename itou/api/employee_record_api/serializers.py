...
class _API_PersonSerializer(serializers.Serializer):
    ...
    NIR = serializers.CharField(
        source="job_application.job_seeker.jobseeker_profile.nir",
        label="Le NIR renseigné sur le profil du demandeur d'emploi",
    )
    NTT = serializers.CharField(
        source="ntt",
        label="Le NTT renseigné sur la fiche salarié",
        help_text="Nécessaire pour les salariés sans NIR/NIA ou avec un NIA commençant par 7 ou 8.",
    )
    ...
    salarieNIR = serializers.SerializerMethodField(
        label="Le NIR/NIA/NTT qui serait transmis à l'ASP",
        help_text="NTT pour les salariés sans NIR/NIA ou avec un NIA commençant par 7 ou 8, sinon NIR/NIA.",
    )
...
class EmployeeRecordAPISerializer(serializers.Serializer):
    ...
    typeMouvement = serializers.CharField(default=MovementType.CREATION, read_only=True)  # Required
