...
def get_invitation_status(invitation_dict):
    if invitation_dict["clicked"]:
        return InvitationStatus.OPENED
    if invitation_dict["delivery_status"]:
        if invitation_dict["delivery_status"] in RDV_I_INVITATION_DELIVERED_STATUSES:
            return InvitationStatus.DELIVERED
        if invitation_dict["delivery_status"] in RDV_I_INVITATION_NOT_DELIVERED_STATUSES:
            return InvitationStatus.NOT_DELIVERED
        logger.error(f"Invalid RDV-I invitation status: '{invitation_dict['delivery_status']}' not in supported list")
