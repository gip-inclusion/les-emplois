...
class InvitationRequest(models.Model):
    ...
    def email_invitation(self):
        return next(iter([invit for invit in self.invitations.all() if invit.is_email]), None)

    @property
    def sms_invitation(self):
        return next(iter([invit for invit in self.invitations.all() if invit.is_sms]), None)
...
class Appointment(models.Model):
    ...
    invitation_request = models.ForeignKey(
        "InvitationRequest",
        null=True,
        verbose_name="demandeur d'invitation",
    ...
    starts_at = models.DateTimeField("commence le")
    duration = models.DurationField("durée")
    cancelled_at = models.DateTimeField("annulé le", null=True)
...
class Participation(models.Model):
    ...
    def get_status_display(self):
        if self.status == self.Status.UNKNOWN:
            if self.appointment.starts_at > timezone.now():
                return "RDV à venir"
