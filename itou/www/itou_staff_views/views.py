...
def export_er_3437(request):
    employee_record_qs = EmployeeRecord.objects.filter(
    ...
    def content():
        yield er_3437_export_spec.keys()
        for obj in employee_record_qs.iterator():
            yield export_row(er_3437_export_spec, obj)
    ...
    return StreamingHttpResponse(
        ...
        headers={
            "Content-Disposition": content_disposition_header(
                as_attachment=True,
                filename=f"export_er_3437_{get_export_ts()}.csv",
            ),
        },
        streaming_content=(writer.writerow(row) for row in content()),
    )
