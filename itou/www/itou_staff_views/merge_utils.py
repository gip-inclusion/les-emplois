...
def handle_job_seeker_assignment(model, from_user, to_user):
    ...
    to_user_assignments = {
        assignment.job_seeker_id: assignment for assignment in model.objects.filter(prescriber=to_user)
    }
    updated_pks = []
    moved_pks = []
    for from_user_assignment in from_user_assignments:
        if to_user_assignment := to_user_assignments.get(from_user_assignment.job_seeker.pk):
            ...
            if from_user_assignment.prescriber_organization is None:
                # Cannot have an assignment without prescriber nor organization
                from_user_assignment.delete()
            else:
                JobSeekerAssignment.objects.filter(pk=from_user_assignment.pk).update(prescriber=None)
        else:
            moved_pks.append(from_user_assignment.pk)
            from_user_assignment.prescriber = to_user
            from_user_assignment.save()
