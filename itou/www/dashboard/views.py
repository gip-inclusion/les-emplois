...
def dashboard(request, template_name="dashboard/dashboard.html"):
    context = {
        ...
        "can_view_gps_card": is_allowed_to_use_gps(request.user) and not show_gps_as_a_nav_entry(request),
    ...
    if request.user.is_employer:
        ...
        if current_org := request.current_organization:
            context["stalled_job_seekers"] = User.objects.linked_job_seeker_ids(
                request.user,
