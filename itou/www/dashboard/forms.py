    ...
    def save(self):
        ...
        for field_name, value in self.cleaned_data.items():
            if field_name.startswith("category-"):
                continue
            if not value:
                notification = NotificationRecord.objects.get(notification_class=field_name)
                if not self.cleaned_data[f"category-{slugify(notification.category)}-all"]:
                    disabled_notifications.append(notification)
