...
def reset_request_init(request, template_name="otp_views/reset_request_init.html"):
    ...
    if request.method == "POST":
        ...
        AuditTrail.objects.create(
            AuditTrailEventType.SECOND_FACTOR_RESET_REQUEST,
            request,
            data={"itou_2fa_reset_request_pk": reset_request.pk},
        )
        notify_2fa_reset_request_init(reset_request, getattr(request, "organizations", None))
...
def reset_request_do_reset(request, nonce, template_name="otp_views/reset_request_do_reset.html"):
    reset_request = get_object_or_404(ResetRequest.objects.select_for_update(of=("self",)), nonce=nonce)
