...
class ItouLoginForm(LoginForm):
    ...
    def clean(self):
        ...
        user = User.objects.filter(email=login).first() if login else None
        if (
            user
            ...
            # TODO(alaurent): Update this behaviour on 2025/12 depending on if ProConnect has less issues
