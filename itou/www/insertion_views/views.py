...
def orientation_details_for_service_provider(request):
    ...
    if not link.is_valid:
        ...
        if request.method == "POST":
            ...
            new_link.email_new_link_for_structure.send()
            messages.success(
                request,
                "Nouveau lien généré||"
...
def document_download(request, document_id):
    link = None
    if token_id := request.GET.get("token"):
        is_valid_filter = Q(
            created_at__gte=timezone.now()
            - datetime.timedelta(seconds=insertion_models.OrientationProcessLink.MAX_VALIDITY_SECONDS)
        )
        link = get_object_or_404(
        ...
        if request.from_prescriber:
            ...
            raise Http404
        orientation_subquery = insertion_models.Orientation.objects.filter(Q(**orientation_filter))
    ...
    if token_id:
        logger.info("orientation document_download orientation_process_link=%s document=%s", link.pk, document.pk)
    else:
        logger.info("orientation document_download user=%s document=%s", request.user.pk, document.pk)
    return HttpResponseRedirect(
