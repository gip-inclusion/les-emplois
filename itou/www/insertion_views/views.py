    ...
    if not (job_seeker_public_id := request.GET.get("job_seeker_public_id")):
        ...
        return HttpResponseRedirect(
            reverse(
                ...
                query={
                    "tunnel": "orientation",
                    "from_url": reverse("insertion_views:service_card", kwargs={"service_uid": service.uid}),
...
class OrientationConfirmationView(LoginRequiredMixin, TemplateView):
...
class OrientationErrorView(LoginRequiredMixin, TemplateView):
...
class OrientationWizardView(LoginRequiredMixin, WizardView):
    ...
    def _post_documents_step(self, request, *args, **kwargs):
        ...
        cleaned = self.form.cleaned_data.copy()
        try:
            ...
            return self._redirect_on_submission_error(request, "safe_upload")

        self.wizard_session.set(self.step, cleaned)
        ...
        try:
            self.dora_client.create_orientation(payload)
        except DoraAPIException:
