...
class ApplicationBaseView(ApplyStepBaseView):
    ...
    def get_eligibility_setp_url(self):
        ...
        if self.company.kind != CompanyKind.GEIQ:
            ...
            if not any(bypass_eligibility_conditions):
                return add_url_params(
                    reverse("eligibility_views:update", kwargs={"public_id": self.job_seeker.public_id}),
                    {"company_pk": self.company.pk, "reset_url": self.get_reset_url()},
                )
...
class ApplicationJobsView(ApplicationBaseView):
    ...
    def get_next_url(self):
        if eligibility_url := self.get_eligibility_setp_url():
            return eligibility_url
        return reverse(
