...
def details_for_company(request, job_application_id, template_name="apply/process_details_company.html"):
        ...
        .annotate(
            ...
            upcoming_participations=Count(
                ...
                filter=Q(
                    ...
                    job_seeker__rdvi_participations__appointment__starts_at__gt=timezone.now(),
                ),
            ),
        )
    )
    ...
    participations = job_application.job_seeker.rdvi_participations.filter(
        appointment__company=job_application.to_company
            ...
            with transaction.atomic():
                ...
                for invitation in response_data["invitations"]:
                    ...
                    if invitation["delivered_at"]:
                        try:
                            extra_kwargs["delivered_at"] = datetime.datetime.fromisoformat(invitation_status)
