...
from itou.job_applications.models import JobApplication, JobApplicationThreadedComment, PriorAction
...
class JobApplicationAddCommentForCompanyForm(forms.ModelForm):
    ...
    message = forms.CharField(label="Ajouter un commentaire", required=False, widget=forms.Textarea(), strip=True)

    class Meta:
        model = JobApplicationThreadedComment
        fields = ["message"]
    ...
    def save(self, *args, **kwargs):
        ...
        comment.job_application = self.job_application
        comment.created_by = self.created_by
        comment.save()
        return comment
