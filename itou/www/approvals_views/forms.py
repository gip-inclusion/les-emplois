...
class CreateProlongationForm(forms.ModelForm):
    ...
    def __init__(self, *args, approval, siae, **kwargs):
        ...
        other_prolongation_duration = Prolongation.objects.get_cumulative_duration_for(
            approval.pk, ignore=self.instance.pk
        )
        disabled_choices = set()
        for reason, _reason_label in self.fields["reason"].choices:
            if (
                max_cumulative_duration := Prolongation.PROLONGATION_RULES.get(reason, {}).get(
                    "max_cumulative_duration"
                )
            ) is not None:
                if max_cumulative_duration < other_prolongation_duration:
                    disabled_choices.add(reason)

        if disabled_choices:
            # Make those options appear as disabled
            self.fields["reason"].widget.disabled_choices = disabled_choices
            # Prevent those options to be accepted
            self.fields["reason"]._choices = [
                (k, v) for k, v in self.fields["reason"]._choices if k not in disabled_choices
            ]
