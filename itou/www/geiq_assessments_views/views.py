...
def upload_action_financial_assessment(
    ...
    assessments = Assessment.objects.filter(companies=request.current_organization, submitted_at__isnull=True)
    assessment = get_object_or_404(assessments, pk=pk)
...
def assessment_details_for_institution(
    ...
    if request.method == "POST":
        ...
        if action is InstitutionAction.REVIEW:
            if request.current_organization.kind == InstitutionKind.DREETS_GEIQ and assessment.final_reviewed_at:
                raise PermissionDenied
            now = timezone.now()
            if not assessment.reviewed_at:
            ...
            if request.current_organization.kind != InstitutionKind.DREETS_GEIQ or assessment.final_reviewed_at:
                raise PermissionDenied
