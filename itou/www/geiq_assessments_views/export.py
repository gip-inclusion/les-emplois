...
def _get_employee_criteria(contract):
    criteria = []
    for criterium in contract.employee.other_data.get("statuts_prioritaire", []):
        level = criterium.get("niveau")
        libelle = criterium.get("libelle")
...
EMPLOYEE_CONTRACT_XLSX_FORMAT = {
    ...
    "Temps plein": with_format(Format.BOOLEAN, lambda contract: contract.other_data.get("is_temps_plein", "")),
    ...
    "Nombre de jours du contrat réalisée": with_format(Format.INTEGER, lambda contract: contract.duration().days),
    "Nombre de jours dans l’année du bilan": with_format(
        Format.INTEGER, lambda contract: contract.nb_days_in_campaign_year
    ),
    ...
    "Contrat professionnel expérimental": with_format(
        Format.BOOLEAN, lambda contract: contract.other_data.get("is_contrat_pro_experimental", "")
    ),
    "Contrat signé dans le cadre d’une clause d’insertion": with_format(
        Format.BOOLEAN, lambda contract: contract.other_data.get("signer_cadre_clause_insertion", "")
    ),
    ...
    "Mise en situation professionnelle": with_format(
        Format.BOOLEAN, lambda contract: contract.other_data.get("mise_en_situation_professionnelle_bool")
    ),
    ...
    "Titulaire d’un bac général": with_format(
        Format.BOOLEAN, lambda contract: contract.employee.other_data.get("is_bac_general", "")
    ),
    "Qualification visée": with_format(
        Format.TEXT, lambda contract: get_libelle(contract.other_data.get("qualification_visee", {}))
    ),
    "Type de qualification visé": with_format(
    ...
    "Qualification obtenue": with_format(
        Format.BOOLEAN, lambda contract: contract.other_data.get("is_qualification_obtenue", "")
    ),
    ...
    "Type de qualification obtenu": with_format(
    ...
    "CDD/CDI refusé": with_format(Format.BOOLEAN, lambda contract: contract.other_data.get("is_refus_cdd_cdi", "")),
    "Date de rupture anticipée": with_format(
        Format.DATE,
        lambda contract: contract.end_at if contract.end_at != contract.planned_end_at else "",
    ),
...
def export_format_for_user_kind(user_kind):
    ...
    if user_kind == UserKind.EMPLOYER:
        user_fields["Demande d’aide"] = with_format(Format.BOOLEAN, lambda contract: contract.allowance_requested)
    elif user_kind == UserKind.LABOR_INSPECTOR:
        user_fields["Éligible à l’aide"] = with_format(Format.BOOLEAN, lambda contract: contract.allowance_granted)
