...
class AddView(UserPassesTestMixin, WizardView):
    ...
    def done(self, *args, **kwargs):
        ...
        if approval.user.jobseeker_profile.lack_of_nir_reason is LackOfNIRReason.NIR_ASSOCIATED_TO_OTHER:
            return reverse(
...
def missing_employee(request, template_name="employee_record/missing_employee.html"):
    ...
    back_url = reverse(
        ...
        query={"reset_url": get_safe_url(request, "back_url", fallback_url=reverse("employee_record_views:list"))},
    )
...
def create_step_4(request, job_application_id, template_name="employee_record/create.html"):
    ...
    if request.method == "POST" and form.is_valid():
        if not job_application.job_seeker.jobseeker_profile.nir and (ntt := get_session_ntt(request, job_application)):
            # Retrieve NTT from step 1 if needed
            form.employee_record.ntt = ntt
...
def nir_already_used(request, job_seeker_public_id, template_name="employee_record/nir_already_used.html"):
    ...
    if job_seeker.jobseeker_profile.lack_of_nir_reason is not LackOfNIRReason.NIR_ASSOCIATED_TO_OTHER:
