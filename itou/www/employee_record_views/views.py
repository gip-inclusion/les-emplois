...
def list_employee_records(request, template_name="employee_record/list.html"):
    ...
    one_siret_different_from_company = (
        data.filter(status=Status.PROCESSED).exclude(siret__in=F("siret_from_asp_source")).exists()
    )
    ...
    context = {
        ...
        "one_siret_different_from_company": one_siret_different_from_company,
    }
