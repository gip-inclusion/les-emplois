...
class NewEmployeeRecordJobSeekerForm(JobSeekerProfileModelForm):
    ...
    ntt = forms.CharField(
        ...
        required=False,
        max_length=40,  # 15 + 6 white spaces
        strip=True,
    ...
    def __init__(self, *args, siren, editor=None, back_url=None, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def clean(self):
        super().clean()
        # Avoid unique validation in post_clean for NIR field: we do it ourselves here
        ...
        if self.cleaned_data.get("lack_of_nir"):
            if ntt := self.cleaned_data.get("ntt"):
                ...
                self.add_error(
                    "ntt",
                    forms.ValidationError(
                        "Le numéro technique temporaire obligatoire si vous ne disposez pas du NIR ou du NIA."
                    ),
                )
        else:
