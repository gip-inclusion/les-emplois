...
class BaseJobSeekerDetailView(UserPassesTestMixin, ReadonlyViewMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        is_in_overview_test_perimeter = bool(get_jobseeker_overview_data(self.request, self.object))
        show_overview_tab = (
            is_in_overview_test_perimeter
            and self.request.current_organization.department == settings.OVERVIEW_TAB_TEST_DEPARTMENT
        )
        ...
        return super().get_context_data(**kwargs) | {
            ...
            "is_in_overview_test_perimeter": is_in_overview_test_perimeter,
...
def get_jobseeker_overview_data(request, job_seeker):
    ...
    if (
        ...
        and request.current_organization.department
...
def create_or_edit_assignment(
    request, public_id, assignment_pk=None, template_name="job_seekers_views/assignment_edit.html"
):
    ...
    if assignment_pk:
        ...
        # Cannot create an assignment if the current assignment has not ended
        # Instead, redirect to the active assignment edition view
        if active_assignment:
            return HttpResponseRedirect(
                reverse(
                    "job_seekers_views:edit_assignment",
                    kwargs={"public_id": public_id, "assignment_pk": active_assignment.pk},
    ...
    context = {
        "form": form,
        "job_seeker": job_seeker,
        "assignment": assignment if assignment_exists else None,
        "back_url": back_url,
...
class JobSeekerBaseView(ExpectedJobSeekerSessionMixin, TemplateView):
    ...
    def get_exit_url(self, job_seeker, created=False, active_assignment=None):
        ...
        if self.standalone_creation:
            ...
            elif active_assignment and active_assignment.advisor == self.request.user:
                return reverse(
                    "job_seekers_views:edit_assignment",
                    kwargs={"public_id": job_seeker.public_id, "assignment_pk": active_assignment.pk},
                )
            else:
                return reverse("job_seekers_views:create_assignment", kwargs={"public_id": job_seeker.public_id})
        ...
        return self.get_apply_url(view_name, job_seeker)
    ...
    def get_active_assignment(self, job_seeker):
        # Retrieve the previous url: if the user was last on the list view, we only want their own assignments
        from_all_coworkers = self.get_reset_url() == reverse("job_seekers_views:list_organization")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if self.form.data.get("confirm"):
                return HttpResponseRedirect(self.get_exit_url(job_seeker, active_assignment=active_assignment))
