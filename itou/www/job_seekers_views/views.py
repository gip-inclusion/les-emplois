...
class CreateJobSeekerStepBaseView(LoginRequiredMixin, SessionNamespaceRequiredMixin, TemplateView):
    ...
    def setup(self, request, *args, **kwargs):
        ...
        super().setup(request, *args, **kwargs)
        # TODO(ewen): how to check that user is sender?
        ...
        if end_url := request.GET.get("end_url"):
            self.job_seeker_session.update({"end_url": end_url})
...
class CreateJobSeekerCheckNIRView(CreateJobSeekerStepBaseView):
...
class SearchByEmailView(CreateJobSeekerStepBaseView):
    ...
    def get_back_url(self):
        return reverse("job_seekers_views:check_nir", kwargs={"session_uuid": self.job_seeker_session.name})
...
class CreateJobSeekerForSenderBaseView(CreateJobSeekerStepBaseView):
    def get_back_url(self):
...
class CreateJobSeekerStep1View(CreateJobSeekerForSenderBaseView):
