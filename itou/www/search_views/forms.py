...
class NewSavedSearchForm(forms.ModelForm):
    ...
    def clean(self):
        super().clean()
        saved_searches_count = SavedSearch.objects.filter(user=self.user).count()
        if saved_searches_count >= self.MAX_SAVED_SEARCHES_COUNT:
            raise forms.ValidationError(
                f"Vous ne pouvez pas enregistrer plus de {self.MAX_SAVED_SEARCHES_COUNT} recherches. "
