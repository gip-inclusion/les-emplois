...
def search_services_results(request, template_name="search/services/results.html"):
    ...
    job_seeker = None
    if not api_error:
        orientation_jwt = get_orientation_jwt(request)
        if orientation_jwt and (job_seeker_uid := request.GET.get("job_seeker_public_id")):
            with contextlib.suppress(User.DoesNotExist, ValidationError):
                job_seeker = User.objects.get(kind=UserKind.JOB_SEEKER, public_id=job_seeker_uid)
