...
class Commune(PrettyPrintMixin, AbstractPeriod):
    ...
    # Also, if someday the synchronization makes the INSEE city disappear from a Commune, we can "block"
    # and notify the users, asks the ASP for a new referential, and so on.
    city = models.ForeignKey("cities.City", on_delete=models.CASCADE, verbose_name="ville INSEE", null=True)
