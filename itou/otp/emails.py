...
def notify_2fa_reset_request_init(reset_request, user_orgs):
    """Notify both org admins AND the user about a 2fa reset request."""
    admins = get_org_admins(user_orgs, reset_request.user)
    for admin in admins:
        email = get_email_message(
            to=[admin.email],
            context={
                ...
                "user": admin,
                "request_is_suspicious": reset_request.is_suspicious,
            },
...
def notify_2fa_reset_request_accepted(reset_request):
    ...
    email = get_email_message(
        ...
        context={
            ...
            "self_cancel_url": get_absolute_url(reverse("otp_views:reset_request_self_cancel")),
            "OTP_RESET_LINK_VALIDITY": settings.OTP_RESET_LINK_VALIDITY,
        },
...
def notify_2fa_reset_request_done(reset_request):
...
def notify_2fa_reset_request_denied(reset_request):
