...
from itou.otp.emails import (
    notify_mfa_reset_request_accepted,
    notify_mfa_reset_request_denied,
    notify_mfa_reset_request_done,
)
...
class ResetRequestQuerySet(models.QuerySet):
    ...
    def cancellable(self):
        return self.filter(state__in=(ResetRequestState.PENDING, ResetRequestState.ACCEPTED))

    def _expired_pending_requests(self):
        return self.filter(
            state=ResetRequestState.PENDING, created_at__lt=timezone.now() - settings.MFA_RESET_REQUEST_VALIDITY
        )

    def _expired_accepted_requests(self):
        ...
        return self.filter(
            ...
            created_at__lt=timezone.now() - settings.MFA_RESET_REQUEST_VALIDITY,
            updated_at__lt=timezone.now() - settings.MFA_RESET_LINK_VALIDITY,
        )

    def expired_requests(self):
        return self._expired_pending_requests().union(self._expired_accepted_requests())
...
class ResetRequest(xwf_models.WorkflowEnabled, models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                name="unique_%(class)s_open_for_user",
                fields=["user"],
                condition=models.Q(state__in=[ResetRequestState.PENDING, ResetRequestState.ACCEPTED]),
                violation_error_message="Une demande de réinitialisation est déjà en cours pour votre compte.",
            ),
        ]
    ...
    token = models.CharField(default=secrets.token_hex, editable=False)
    ...
    def accept(self, *, actor=None, msg=None):
        notify_mfa_reset_request_accepted(self)

    @xwf_models.transition()
    def deny(self, *, actor=None, msg=None):
        notify_mfa_reset_request_denied(self)
    ...
    def reset_devices(self, *, actor=None, msg=None) -> int:
        ...
        notify_mfa_reset_request_done(self)
    ...
    def check_reset_devices(self):
        ...
        return self.updated_at + settings.MFA_RESET_LINK_VALIDITY > timezone.now()

    @xworkflows.transition_check(ResetRequestTransition.RESEND)
    def check_resend(self):
        """Enforce reset request validity."""
        return self.created_at + settings.MFA_RESET_REQUEST_VALIDITY > timezone.now()

    @property
    def reset_link(self):
        return get_absolute_url(reverse("otp_views:reset_request_do_reset", args=(self.token,)))

    def _credibility_according_to_last_connections(self) -> float:
        ...
        try:
            event = AuditTrail.objects.get(
                ...
                user=self.user,
                data__itou_mfa_reset_request_pk=self.pk,
            )
        except AuditTrail.DoesNotExist:  # Too old, had been removed
            return 0.5  # A neutral result
    ...
    def credibility(self) -> float:
        ...
        Gives a value between 0 (the request does not look legit) and
        1 (it looks legit).
        ...
        return fmean(values, weights=[80, 20])
