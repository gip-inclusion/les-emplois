...
class City(models.Model):
    ...
    post_codes = ArrayField(
        models.CharField(max_length=5), verbose_name=_("Codes postaux"), blank=True
    )
    code_insee = models.CharField(
        verbose_name=_("Code INSEE"), max_length=5, unique=True
    )
...
def find_suspicious_siae_cities():
    ...
    for siae in siaes:
        try:
            City.objects.get(
                slug=slugify(f"{siae.city}-{siae.department}"),
                department=siae.department,
            )
        except City.DoesNotExist:
            print("-" * 80)
            print(
                f"No entry in City() for SIAE {siae.siret} - {siae.name} in {siae.city} - {siae.department}"
            )
