...
class JobApplicationNotifyPoleEmploiIntegrationTest(TestCase):
    ...
    def test_invalid_job_seeker_for_pole_emploi(self):
        ...
        """
        now = timezone.now()
        job_application = JobApplicationWithApprovalFactory(job_seeker__nir="")
    ...
    # Here we mock pole_emploi_api_client.expires_at in order to "reset" it, since
    # the main pole_emploi_api_client keeps a state in it. This test explicitly needs
    # the token to be fetched in order to test a Timeout on it, so there's that.
