...
class JobApplicationQuerySet(models.QuerySet):
    def is_active_company_member(self, user):
        return self.filter(Exists(CompanyMembership.objects.filter(user=user, company=OuterRef("to_company"))))
    ...
    def prescriptions_of(self, user, organization=None):
        ...
        raise ValueError(f"Invalid organization kind: {type(organization)}")
    ...
    def visible_by_employers(self):
        ...
        """
        visibility_date_limit = timezone.now() - APPLICATIONS_VISIBILITY_FOR_EMPLOYERS
