...
class JobApplicationQuerySet(models.QuerySet):
    ...
    def eligible_as_employee_record(self, siae):
        ...
        exclude_same_asp_and_approval = Subquery(
            self.exclude(to_siae=siae, employee_record__status=employeerecord_enums.Status.DISABLED).filter(
...
class JobApplication(xwf_models.WorkflowEnabled, models.Model):
    ...
    def notify_pole_emploi(self, at=None, with_delay=False):
        ...
        if (
            not self.job_seeker.first_name
            or not self.job_seeker.last_name
            or not self.job_seeker.nir
            or not self.job_seeker.birthdate
        ):
