...
{% load str_filters %}
{% block body %}
{% if job_application.sender.title %}Bonjour {{ job_application.sender.get_title_display }} {{ job_application.sender.last_name|upper }},{% else %}Bonjour {{ job_application.sender.get_inverted_full_name }},{% endif %}

{% block header %}L’entreprise {{ job_application.to_company.display_name }} vous propose d’utiliser Diagoriente pour valoriser les expériences de votre candidat : {{ job_application.job_seeker.get_inverted_full_name|mask_unless:can_view_personal_information }}.
