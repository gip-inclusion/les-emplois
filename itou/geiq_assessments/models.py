...
class Assessment(models.Model):
    ...
    # XXX: should it be called "final_reviewed_at" ?
    ...
    # XXX: should we also have a "final_reviewed_by_institution" ?
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                condition=(
                    ...
                    | models.Q(
                        dreets_reviewed_at__isnull=False,
                        dreets_reviewed_by__isnull=False,
                    )
                ),
            ),
        ]
    ...
    def get_allowance_stats_for_institution(self, for_assessment_details=False):
        selected_contract_filter = Q(allowance_granted=True)
        extra_stats = (
            {
                "apprenticeship_contracts_nb": Count(
                    "pk", filter=Q(allowance_granted=True, other_data__nature_contrat__libelle_abr="CAPP")
                ),
                "apprenticeship_contracts_submitted_nb": Count(
                    "pk", filter=Q(allowance_requested=True, other_data__nature_contrat__libelle_abr="CAPP")
                ),
                "professionalization_contracts_nb": Count(
                    "pk", filter=Q(allowance_granted=True, other_data__nature_contrat__libelle_abr="CPRO")
                ),
                "professionalization_contracts_submitted_nb": Count(
                    "pk", filter=Q(allowance_requested=True, other_data__nature_contrat__libelle_abr="CPRO")
                ),
                "contracts_with_3_months_in_assessment_year_nb": Count(
                    "pk", filter=Q(allowance_granted=True, with_3_months_in_assessment_year=True)
                ),
                "contracts_with_3_months_in_assessment_year_submitted_nb": Count(
                    "pk", filter=Q(allowance_requested=True, with_3_months_in_assessment_year=True)
                ),
            }
        ...
        return EmployeeContract.objects.filter(employee__assessment=self, allowance_requested=True).aggregate(
            all_contracts_nb=Count("pk"),  # submitted contracts nb
            contracts_nb=Count("pk", filter=Q(allowance_granted=True)),  # accepted contracts nb
            allowance_of_814_nb=Count("pk", filter=Q(employee__allowance_amount=814, allowance_granted=True)),
            allowance_of_814_submitted_nb=Count(
                "pk", filter=Q(employee__allowance_amount=814, allowance_requested=True)
            ),
            allowance_of_1400_nb=Count("pk", filter=Q(employee__allowance_amount=1400, allowance_granted=True)),
            allowance_of_1400_submitted_nb=Count(
                "pk", filter=Q(employee__allowance_amount=1400, allowance_requested=True)
            ),
    ...
    def get_allowance_stats_for_geiq(self):
        return EmployeeContract.objects.filter(employee__assessment=self).aggregate(
            all_contracts_nb=Count("pk"),
            contracts_nb=Count("pk", filter=Q(allowance_requested=True)),
            allowance_of_0_nb=Count("pk", filter=Q(allowance_requested=True, employee__allowance_amount=0)),
            allowance_of_814_nb=Count("pk", filter=Q(allowance_requested=True, employee__allowance_amount=814)),
            allowance_of_1400_nb=Count("pk", filter=Q(allowance_requested=True, employee__allowance_amount=1400)),
    ...
    def grant_percentage(self):
        if not self.convention_amount:
            return None
        return 100 * self.granted_amount / self.convention_amount
...
class EmployeeContract(models.Model):
    ...
    with_3_months_in_assessment_year = models.BooleanField(verbose_name="plus de 3 mois dans l'année du bilan")
