...
def _pole_emploi_name(name: str, replace_spaces=False, max_len=25) -> str:
...
class PoleEmploiApiClient:
    ...
    def _refresh_token(self, at=None):
        ...
        response = httpx.post(
            ...
            data={
                ...
                "scope": f"application_{settings.API_ESD['KEY']} {AUTHORIZED_SCOPES}",
            },
    ...
    def recherche_individu_certifie(self, job_seeker):
        ...
        data = self._request(
            ...
            {
                "dateNaissance": job_seeker.birthdate.strftime("%Y-%m-%d") if job_seeker.birthdate else "",
                "nirCertifie": job_seeker.nir[:MAX_NIR_CHARACTERS] if job_seeker.nir else "",
                "nomNaissance": _pole_emploi_name(job_seeker.last_name),
                "prenom": _pole_emploi_name(job_seeker.first_name, replace_spaces=True, max_len=13),
            },
        )
    ...
    def mise_a_jour_pass_iae(self, job_application, encrypted_identifier):
        ...
        params = {
            ...
            "statutReponsePassIAE": "A",  # Means "Approved". We used to send refused approvals, but they don’t care
