...
class AbstractEligibilityDiagnosisAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not author_prescriber_organization and not author_company:
            self.add_error(
                "__all__",
        ...
        if author and author_kind:
            if author.is_professional:
                ...
                if (
                    author_company
                    # Allow inactive membership as we may want to fix old diagnoses
                    and not CompanyMembership.include_inactive.filter(company=author_company, user=author).exists()
                ):
