...
class ProlongationDerogationForm(forms.Form):
    ...
    company = forms.ModelChoiceField(
        Company.objects.all(),
        required=True,
        label="Entreprise autorisée à déclarer la prolongation",
        help_text="Saisissez l’ID de la SIAE, ou recherchez-la avec la loupe.",
    )

    def __init__(self, *args, approval, admin_site, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["company"].widget = widgets.ForeignKeyRawIdWidget(FakeRelForRawIdWidget(Company), admin_site)
    ...
    def clean_company(self):
        ...
        if not company.is_subject_to_iae_rules:
            raise ValidationError("Cette entreprise n’est pas une SIAE, elle ne peut pas déclarer de prolongation.")
