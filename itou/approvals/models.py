...
class Prolongation(CommonProlongation):
    ...
    def clean(self):
        ...
        # A prolongation cannot overlap another one for the same PASS IAE.
        ...
        if overlapping_prolongation:
            raise ValidationError(
                f"La période chevauche une prolongation déjà existante pour ce PASS IAE "
                f"{overlapping_prolongation.start_at:%d/%m/%Y} - {overlapping_prolongation.end_at:%d/%m/%Y}."
            )
