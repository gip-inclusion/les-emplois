...
class Command(BaseCommand):
    ...
    **************************************************************************************
    ...
    To debug:
        django-admin import_pe_approvals --file-name=2019_10_base_agrements_PE_with_birthdate.xlsx --dry-run
        django-admin import_pe_approvals --file-name=2019_10_base_agrements_PE_with_birthdate.xlsx --dry-run --verbosity=2

    To populate the database:
        django-admin import_pe_approvals --file-name=2019_10_base_agrements_PE_with_birthdate.xlsx
    """
    ...
    def handle(self, file_name, dry_run=False, **options):
        ...
        for i, row in enumerate(ws.rows):
            ...
            ID_REGIONAL_BENE = str(row[1].value).strip()
            # This XLSX format dropped the leading zeros.
            while len(ID_REGIONAL_BENE) != 8:
                ID_REGIONAL_BENE = f"0{ID_REGIONAL_BENE}"
            ...
            NOM_USAGE_BENE = row[2].value
            if not NOM_USAGE_BENE:
                # Very rare: from 1 to 3 cases.
                continue
            NOM_USAGE_BENE = NOM_USAGE_BENE.strip()
            ...
            NOM_NAISS_BENE = row[4].value
            if not NOM_NAISS_BENE:
                # Very rare: from 1 to 3 cases.
                continue
            NOM_NAISS_BENE = NOM_NAISS_BENE.strip()
            ...
            DATE_DEB_AGR_DEC = row[6].value.date()
            DATE_FIN_AGR_DEC = row[7].value.date()
            ...
            DATE_DE_NAISSANCE = row[8].value
            if not DATE_DE_NAISSANCE or isinstance(DATE_DE_NAISSANCE, str):
                # Very rare: from 1 to 3 cases.
                continue
            DATE_DE_NAISSANCE = DATE_DE_NAISSANCE.date()
            ...
            if not dry_run:
                ...
                pe_approval.birthdate = DATE_DE_NAISSANCE
