...
class Command(BaseCommand):
    ...
    To run:
        django-admin batch_notify_pe --start_date="04/01/2022" --dry-run
    """
    ...
    def batch_notiy_pole_emploi(self, start_date, dry_run: bool):
        """
        ...
        if dry_run:
            ...
            self.stdout.write(f"{approvals.count()} valid approvals start on {start_date.strftime(DATE_FORMAT)}")
        else:
    ...
    def handle(self, start_date_str, dry_run=False, **options):
        ...
        self.batch_notiy_pole_emploi(start_date, dry_run)
