...
def test_full_import_wet_run(caplog, snapshot, apis_mocks):
    ...
    assert Service.objects.get(uid="emplois-de-linclusion--null").extra is None
...
def test_import_soft_deletes_structures_and_services_absent_from_api(apis_mocks):
    ...
    service = Service.include_inactive.get(uid="extra-service")
    ...
    assert structure in list(Structure.include_inactive.all())
...
def test_import_reactivates_reappearing_structures_and_services(apis_mocks):
    ...
    Structure.include_inactive.filter(pk=structure.pk).update(is_active=False)
    Service.include_inactive.filter(pk=service.pk).update(is_active=False)
