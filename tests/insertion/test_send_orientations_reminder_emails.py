...
def test_send_orientation_reminder_email(caplog, snapshot, django_capture_on_commit_callbacks, mailoutbox):
    ...
    no_reminder_datetime = now - datetime.timedelta(
        days=Orientation.REMINDER_EMAIL_DELAY_DAYS - 1, hours=23, minutes=59
    )
    ...
    with freeze_time(now - datetime.timedelta(days=Orientation.PENDING_EXPIRATION_PERIOD_DAYS, minutes=1)):
        # An old orientation that failed to be expired -- we don’t want to indefinitely send reminders
        OrientationFactory(status=OrientationStatus.PENDING)
