...
def test_sync_users_update_only():
    with freeze_time() as frozen_time:
        user = EmployerFactory()
        ...
        assert (
            sync_users([build_user(serialize_user(u), Service.PILOTAGE) for u in [user, new_user]], update_only=True)
            == 1
        )
        assertQuerySetEqual(nexus_user_qs.values_list("pk", flat=True), [f"pilotage--{user.pk}"])
        assert set(nexus_user_qs.values_list("updated_at", flat=True)) == {timezone.now()}
...
def test_sync_memberships(factory):
    with freeze_time() as frozen_time:
        membership = factory()
...
def test_sync_structures(factory, kwargs):
    with freeze_time() as frozen_time:
...
def test_init_full_sync():
    with freeze_time() as frozen_time:
        ...
        assert updated_api_sync.new_start_at == timezone.now()
        assert updated_api_sync.new_start_at != api_sync.new_start_at


def test_complete_full_sync():
    start_at = timezone.now()
    NexusRessourceSyncStatusFactory(service=Service.DORA, new_start_at=start_at)
    ...
    assert api_synced.new_start_at == start_at
    ...
    assert api_synced.new_start_at is None
