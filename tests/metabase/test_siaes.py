...
def test_address_fields():
    ...
    parent = CompanyFactory(subject_to_iae_rules=True, source="ASP", post_code=vannes.post_codes[0])
    antenna = CompanyFactory(subject_to_iae_rules=True, source="USER_CREATED", with_convention=parent.convention)
...
def test_address_fields_of_antenna():
    vannes = create_city_vannes()
    parent = CompanyFactory(subject_to_iae_rules=True, source="ASP")
    antenna = CompanyFactory(
        subject_to_iae_rules=True,
        source="USER_CREATED",
        with_convention=parent.convention,
        post_code=vannes.post_codes[0],
    )
