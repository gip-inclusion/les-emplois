...
class TestLastAssignment:
    ...
    def test_last_assignment(self):
        ...
        job_seeker_assignment_1 = JobSeekerAssignmentFactory(job_seeker=job_seeker, updated_at=updated_at)
    ...
    def test_advisor(self):
        ...
        assignment_with_unknown_advisor = JobSeekerAssignmentFactory(
            assigned_to_unknown_advisor=True, company=CompanyFactory()
        )
        assert assignment_with_unknown_advisor.advisor is None
...
class TestJobSeekerAssignment:
    ...
    def test_str(self, prescriber_organization_factory, company_factory):
        ...
        assert str(assignment) == (
            ...
            f"professional={assignment.professional.pk}, prescriber_organization={organization_id}, "
    ...
    def test_assign_job_seeker(self, with_prescriber_organization, with_company, snapshot):
        ...
        with freezegun.freeze_time("2025-11-14 12:00:01"):
            with assertSnapshotQueries(snapshot(name="assignment creation sql queries")):
                JobSeekerAssignment.objects.upsert_assignment(
                    job_seeker, professional, organization, ActionKind.CREATE
                )
        assignment = JobSeekerAssignment.objects.get()
        assert assignment.job_seeker == job_seeker
        assert assignment.professional == professional
        ...
        with freezegun.freeze_time("2025-11-15 18:00:01"):
            with assertSnapshotQueries(snapshot(name="assignment update sql queries")):
                JobSeekerAssignment.objects.upsert_assignment(job_seeker, professional, organization, ActionKind.APPLY)
        assignment = JobSeekerAssignment.objects.get()
        assert assignment.job_seeker == job_seeker
        assert assignment.professional == professional
        assert assignment.prescriber_organization == prescriber_organization
    ...
    def test_assigned_to(self, organization_factory, membership_factory):
        ...
        archived_assignment_no_organization = JobSeekerAssignmentFactory(professional=prescriber, ended=True)
        ...
        # Make sure a job seeker isn't picked up as long as they have at least one active assignment
        JobSeekerAssignmentFactory(
    ...
    def test_display_mode(self):
        ...
        # With a prescriber organization and no membership
        assignment = JobSeekerAssignmentFactory(prescriber_organization=PrescriberOrganizationFactory())
        assignment.is_still_member = False
        assert assignment.display_mode == JobSeekerAssignmentDisplayMode.ACTIVE_WITH_ORG_NO_MEMBERSHIP
