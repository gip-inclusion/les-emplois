...
def test_admin_membership(admin_client, membership_factory):
    admin_active_membership = membership_factory(user__identity_provider=IdentityProvider.DJANGO)
    ...
    membership_field_name = active_membership._meta.model._meta.get_field("user").remote_field.name
    ...
    assert not admin_active_membership.is_active
    assert not admin_active_membership.is_admin
    active_membership.refresh_from_db()
    assert not active_membership.is_active
    assert not active_membership.is_admin
    inactive_membership.refresh_from_db()
    assert not inactive_membership.is_active
    assert not inactive_membership.is_admin
