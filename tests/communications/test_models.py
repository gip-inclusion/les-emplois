...
class NotificationSettingsModelTest(TestCase):
    ...
    def test_str(self):
        assert str(NotificationSettings.get_or_create(self.job_seeker)) == "Paramètres de notification de John DOE"
        assert (
            str(NotificationSettings.get_or_create(self.employer, self.employer_structure))
            == f"Paramètres de notification de Alice DOE ({self.employer_structure})"
        )
        assert (
            str(NotificationSettings.get_or_create(self.prescriber, self.prescriber_structure))
