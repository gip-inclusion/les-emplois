...
from tests.pg_storage.factories import TaskFactory


@pytest.fixture(name="pg_storage")
def pg_storage_fixture():
    storage = PostgresStorage(name=settings.HUEY["name"])
    return storage
...
class TestPostgresStorageEnqueue:
    ...
    def test_enqueue_with_priority(self, pg_storage, priority):
        ...
        assert task.queue == settings.HUEY["name"]


class TestPostgresStorageDequeue(TransactionTestCase):
    def setUp(self):
        self.storage = PostgresStorage(name=settings.HUEY["name"])
