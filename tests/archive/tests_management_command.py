...
class TestAnonymizeProfessionalManagementCommand:
    ...
    def test_anonymize_professionals_after_grace_period(
        ...
        # Check logs
        main_log = next(
            message for message in caplog.messages if "Anonymized professionals after grace period, count" in message
        )
        assert main_log == snapshot(name="anonymized_professionals_after_grace_period_log")
        detail_log = next(message for message in caplog.messages if "Included in this coun" in message)
        assert detail_log == snapshot(name="anonymized_professionals_after_grace_period_detail_log")
    ...
    def test_num_queries(self, django_capture_on_commit_callbacks, snapshot):
        ...
        num_queries = (
            1  # reset_notified_professionals_with_recent_activity() : UPDATE "users_user"
            + 2  # SAVEPOINT / RELEASE SAVEPOINT
            + 1  # get_users: select users_user
            + 4  # get_users_to_anonymize_and_delete: user/companymembership/institutionmembership/prescribermembership
            + 9  # ArchiveUser(user).send: insert into emails_email
            + 1  # anonymize_and_delete_professionals: insert into archive_anonymizedprofessional
            + 1  # anonymize_and_delete_professionals: select user
            + 44  # anonymize_and_delete_professionals: select related objects
            + 15  # anonymize_and_delete_professionals: delete users in cascade
        )
