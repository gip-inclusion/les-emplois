...
def test_displayed_tabs(client, with_membership):
    ...
    assertion = assertContains if with_membership else assertNotContains
...
@pytest.mark.parametrize("url", [reverse("job_seekers_views:list"), reverse("job_seekers_views:list_organization")])
def test_job_seekers_order(client, subtests, url):
    prescriber = PrescriberFactory()
    c_d_job_seeker = JobApplicationFactory(
        sender=prescriber,
        job_seeker__created_by=prescriber,
