...
class TestOverviewTab:
    ...
    def setup(self, settings):
        self.department = "13"
        settings.OVERVIEW_TAB_TEST_DEPARTMENT = self.department
    ...
    def test_forbidden_access(self, client):
        ...
        for user in [
            job_seeker,
            PrescriberFactory(
                membership__organization__authorized=False, membership__organization__department=self.department
            ),
            PrescriberFactory(membership__organization__authorized=True, membership__organization__department="86"),
            EmployerFactory(),
            LaborInspectorFactory(),
        ]:
            client.force_login(user)
