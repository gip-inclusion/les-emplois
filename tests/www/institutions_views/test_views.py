...
class TestMembers:
    ...
    def test_permission(self, client, factory, access):
        ...
        response = client.get(reverse("institutions_views:members"))
        if access:
    ...
    def test_members(self, client, snapshot, monkeypatch):
        ...
        invite_url = reverse("invitations_views:invite_labor_inspector")
        assertContains(response, invite_url)

        LaborInspectorInvitationFactory(institution=institution)
        response = client.get(url)
    ...
    def test_deactivate_user(self, caplog, client, mailoutbox, snapshot):
        ...
        assert "Un administrateur vous a retiré d'une structure sur La plateforme de l’inclusion" in email.body
