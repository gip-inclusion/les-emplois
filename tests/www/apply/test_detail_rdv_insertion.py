...
class ProfileLoginTestMixin:
    def _force_profile_login(self, client, profile):
        if profile == "employer":
...
class TestRdvInsertionAppointmentsList(ProfileLoginTestMixin):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("employer", "details_for_company", "process_details_company"),
            ("job_seeker", "details_for_jobseeker", "process_details"),
        ],
    )
    def test_details_should_not_include_appointments_tab_if_not_configured_and_without_upcoming_appointments(
        self, client, profile, view_name, template_name
    ):
        ...
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("employer", "details_for_company", "process_details_company", True),
            ("job_seeker", "details_for_jobseeker", "process_details", False),
        ],
    )
    def test_details_should_include_appointments_tab_if_configured_and_without_upcoming_appointments(
        self, client, profile, view_name, template_name, contains_tab
    ):
        self.participation.appointment.delete()
    ...
    def test_appointments_tab_should_display_upcoming_appointments(self, client, profile, view_name):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
    ...
    def test_appointments_listing_display(self, client, snapshot, profile, view_name):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
    ...
    def test_appointments_listing_display_previous_appointments(self, client, snapshot, profile, view_name):
        ParticipationFactory(
    ...
    def test_appointments_listing_status_badges(self, client, snapshot, profile, view_name):
        ParticipationFactory(
            id="22222222-2222-2222-2222-222222222222",
            for_snapshot=True,
            job_seeker=self.job_application.job_seeker,
    ...
    def test_appointments_tooltips(self, client, snapshot, profile, view_name):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
...
@freeze_time("2024-08-01")
class TestRdvInsertionInvitationRequestsList(ProfileLoginTestMixin):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("employer", "details_for_company", True),
            ("job_seeker", "details_for_jobseeker", False),
        ],
    )
    def test_invitations_requests_listing(self, client, snapshot, profile, view_name, invitations_presence):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
    ...
    def test_invite_button_uses_for_detail_endpoint(self, client, profile, view_name, contains_invite_button):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}", kwargs={"job_application_id": self.job_application.pk}))
