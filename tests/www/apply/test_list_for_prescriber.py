...
def test_list_for_prescriber(client):
    """
    ...
    exports_link = unquote(
        add_url_params(
            reverse("apply:list_for_prescriber_exports"), {"back_url": reverse("apply:list_for_prescriber")}
        )
    )
    ...
    job_application_link = unquote(
        add_url_params(
            ...
            {"back_url": reverse("apply:list_for_prescriber")},
        )
    )
    ...
    company_link = unquote(
        add_url_params(job_application.to_company.get_card_url(), {"back_url": reverse("apply:list_for_prescriber")})
    )
...
def test_list_for_prescriber_as_unauthorized_prescriber(client):
...
def test_list_for_prescriber_filtered_by_state(client):
    """
    ...
    response = client.get(reverse("apply:list_for_prescriber"), {"states": [job_application.state]})
...
def test_list_for_prescriber_filtered_by_sender(client):
    ...
    response = client.get(reverse("apply:list_for_prescriber"), {"senders": another_prescriber.pk})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
    assert applications[0].sender.id == another_prescriber.pk


def test_list_for_prescriber_filtered_by_job_seeker(client):
    ...
    response = client.get(reverse("apply:list_for_prescriber"), {"job_seekers": [job_seeker.pk]})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
...
def test_list_for_prescriber_filtered_by_company(client):
    prescriber = PrescriberFactory()
    job_application, *others = JobApplicationFactory.create_batch(3, sender=prescriber)
    client.force_login(prescriber)

    response = client.get(reverse("apply:list_for_prescriber"), {"to_companies": [job_application.to_company.pk]})
    applications = response.context["job_applications_page"].object_list
    assert len(applications) == 1
...
def test_list_for_prescriber_filters(client, snapshot):
...
def test_list_for_prescriber_htmx_filters(client):
    prescriber = PrescriberFactory()
...
def test_list_for_prescriber_exports_without_organization(client):
    client.force_login(PrescriberFactory())

    response = client.get(reverse("apply:list_for_prescriber_exports"))
    assert_previous_step(response, reverse("dashboard:index"))
    assertNotContains(response, BESOIN_DUN_CHIFFRE)


def test_list_for_prescriber_exports_with_organization(client):
...
def test_list_for_prescriber_exports_as_pole_emploi_prescriber(client, snapshot):
...
def test_list_for_prescriber_exports_as_employer(client):
    job_application = JobApplicationFactory()
...
def test_list_for_prescriber_exports_back_to_list(client):
    client.force_login(PrescriberFactory())
...
def test_list_for_prescriber_exports_download(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.sender)

    response = client.get(reverse("apply:list_for_prescriber_exports_download"))
    assert 200 == response.status_code
    assert "spreadsheetml" in response.get("Content-Type")


def test_list_for_prescriber_exports_download_as_employer(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.to_company.members.get())
...
def test_list_for_prescriber_exports_download_by_month(client):
    job_application = JobApplicationFactory()
    client.force_login(job_application.sender)

    response = client.get(
        reverse(
            "apply:list_for_prescriber_exports_download",
