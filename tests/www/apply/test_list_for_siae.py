...
def test_list_for_siae_select_applications_batch_archive(client, snapshot):
    ...
    # Only not archived and not archivable job application (bad status)
    simulate_applications_selection([unarchivable_app.pk])
    # No modal & linked button
    assert get_archive_modal() is None
    archive_button = get_archive_button()
    assert pretty_indented(archive_button) == snapshot(name="inactive archive button - bad status")

    # No archivable job app (at least one archived job app)
    for app_list in [
        [archived_app.pk],
        ...
        assert get_archive_modal() is None
        archive_button = get_archive_button()
        assert pretty_indented(archive_button) == snapshot(name="inactive archive button - default")
