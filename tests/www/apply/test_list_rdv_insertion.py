...
class TestRdvInsertionDisplay:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("employer", "list_for_siae", SEE_JOB_APPLICATION_LABEL),
            ("job_seeker", "list_for_job_seeker", SEE_JOB_APPLICATION_LABEL_FOR_JOB_SEEKER),
        ],
    )
    def test_list_no_rdv_insertion_button_when_not_configured(self, client, profile, view_name, job_application_label):
        ...
        response = client.get(reverse(f"apply:{view_name}"))
    ...
    def test_list_rdv_insertion_button_when_configured(self, client, profile, view_name, job_application_label):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}"))
        assertContains(response, job_application_label)
        if profile == "employer":
    ...
    def test_list_rdv_insertion_button_when_configured_and_sent(
        self, client, profile, view_name, job_application_label
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("employer", "list_for_siae", "list_card_body_company"),
            ("job_seeker", "list_for_job_seeker", "list_card_body_jobseeker"),
        ],
    )
    def test_list_no_upcoming_appointments(self, client, profile, view_name, template_name):
    ...
    def test_list_with_one_upcoming_appointment(self, client, profile, view_name, template_name):
        self._force_profile_login(client, profile)
        response = client.get(reverse(f"apply:{view_name}"))
        assertTemplateUsed(response, f"apply/includes/{template_name}.html")
    ...
    def test_list_with_many_upcoming_appointments(self, client, profile, view_name, template_name):
        self._force_profile_login(client, profile)
