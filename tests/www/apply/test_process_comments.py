...
def test_display_in_sidebar_and_tab(client, snapshot):
    ...
    JobApplicationCommentFactory.create_batch(VISIBLE_COMMENTS_COUNT, job_application=job_app)
    # A comment linked to the job app but not supposed to be seen with members of that company
    # (for various reasons such as a faulty tranfer in admin)
    hidden_comment = JobApplicationCommentFactory(
        message="Un commentaire qui ne doit pas être lu.", job_application=job_app, company_id=CompanyFactory().pk
    )
    ...
    assertNotContains(response, hidden_comment.message)
