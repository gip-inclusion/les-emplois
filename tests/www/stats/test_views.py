...
@pytest.mark.parametrize(
    ...
    ],  # Partial accesss by region (configured in can_view_stats_ph utility)
)
def test_stats_ph_state_main_tally_form_overrides(client, snapshot, organization_kind):
    organization = PrescriberOrganizationWithMembershipFactory(
        kind=organization_kind, department="75", authorized=True
    )
    client.force_login(organization.members.get())

    hidden_fields = {"type_prescripteur": organization_kind}

    response = client.get(reverse("stats:stats_ph_state_main"))
    assert response.status_code == 200
    assert response.context["tally_hidden_fields"] == hidden_fields
