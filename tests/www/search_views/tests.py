...
class TestSearchCompany:
    ...
    def test_has_no_active_members(self, client):
        ...
        no_hiring_str = (
            "Cet employeur n'est actuellement pas inscrit sur le site de La plateforme de l’inclusion, "
    ...
    def test_company(self, client, snapshot):
        ...
        assertContains(response, self.applications_open_str)
    ...
    def test_is_not_open_to_spontaneous_applications(self, client):
        ...
        assertNotContains(response, self.applications_open_str)
    ...
    def test_is_not_open_to_spontaneous_applications_but_has_jobs(self, client):
        ...
        # If the employer closes the jobs that are available, it should be as though none are available
        searchable_company.job_description_through.update(is_active=False)

        response = client.get(self.URL, {"city": guerande.slug})
        assertContains(response, f"<h3>{searchable_company.display_name}</h3>")
        assertContains(response, self.no_spontaneous_applications_str)
        assertNotContains(response, self.applications_open_str)
        apply_url = reverse("apply:start", kwargs={"company_pk": searchable_company.pk})
        assertNotContains(response, apply_url)
...
class TestSearchPrescriber:
    ...
    def test_results(self, client):
        ...
        assertContains(
            response,
            "<title>Accompagnement à 15 km du centre de Guérande (44) - La plateforme de l’inclusion</title>",
            html=True,
            count=1,
        )
    ...
    def test_category_filter(self, client):
        ...
        authorized_ccas = PrescriberOrganizationFactory(
            authorized=True,
            kind=PrescriberOrganizationKind.CCAS,
            coords=guerande.coords,
            name="CCAS Habilité",
        )
        unauthorized_ccas = PrescriberOrganizationFactory(
            kind=PrescriberOrganizationKind.CCAS,
            coords=guerande.coords,
            name="CCAS Non Habilité",
        )
        spip = PrescriberOrganizationFactory(
            authorized=True,
            kind=PrescriberOrganizationKind.SPIP,
            coords=guerande.coords,
            name="SPIP Test",
        )
        url = reverse("search:prescribers_results")
        ...
        assertContains(response, spip.name)
        ...
        assertContains(response, authorized_ccas.name)
        assertNotContains(response, unauthorized_ccas.name)
        ...
        assertNotContains(response, spip.name)
        ...
        response = client.get(url, {**params, "category": PrescriberOrganizationCategory.SOCIAL})
        assertContains(response, "1 résultat")
        assertContains(response, authorized_ccas.name)
        assertNotContains(response, unauthorized_ccas.name)
        assertContains(response, "Prescripteur habilité emploi inclusif")

        response = client.get(url, {**params, "category": PrescriberOrganizationCategory.JUSTICE})
        assertContains(response, "1 résultat")
        assertContains(response, spip.name)
        assertNotContains(response, mission_locale.name)
    ...
    def test_form_invalid(self, client):
        ...
        assertContains(
            response,
            "<title>Rechercher un accompagnement - La plateforme de l’inclusion</title>",
            html=True,
            count=1,
        )
