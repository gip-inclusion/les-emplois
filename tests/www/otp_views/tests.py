...
def test_mfa_reset_full_process(client, settings, mailoutbox, django_capture_on_commit_callbacks):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    ItouTOTPDeviceFactory(name="1", user=user)
    create_otp_backup_code(user)
    ...
    assert reset_request.token in email.body

    # Step 3: User clicks the reset link
    assert ItouTOTPDevice.objects.exists()
    assert ItouStaticDevice.objects.exists()
    assert ItouStaticToken.objects.exists()
    response = client.get(reverse("otp_views:reset_request_do_reset", kwargs={"token": reset_request.token}))
...
def test_mfa_reset_cannot_request_twice(client, settings, accept_it):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    ItouTOTPDeviceFactory(name="1", user=user)
    client.force_login(user)
...
@pytest.mark.parametrize("accept_it", [True, False])
def test_mfa_reset_self_cancel(client, settings, accept_it):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    ItouTOTPDeviceFactory(name="1", user=user)
    client.force_login(user)
...
def test_mfa_reset_mail_to_self_when_reset_request_submitted(client, settings, mailoutbox):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    ItouTOTPDeviceFactory(name="1", user=user)
    client.force_login(user)
...
@pytest.mark.parametrize("accept_it", [True, False])
def test_mfa_reset_cleaned_after_successful_login(client, settings, accept_it):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    client.force_login(user)
    device = ItouTOTPDeviceFactory(name="1", user=user)
...
def test_mfa_reset_cannot_use_reset_link_twice(client, settings):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    ItouTOTPDeviceFactory(name="1", user=user)
    client.force_login(user)
...
def test_mfa_reset_link_cannot_be_used_after_expiration(settings, client):
    settings.REQUIRE_OTP_FOR_STAFF = True
    user = ItouStaffFactory(with_verified_email=True)
    device = ItouTOTPDeviceFactory(name="1", user=user)
    client.force_login(user)
    ...
    with mock.patch(
        ...
        return_value=timezone.now() - settings.MFA_RESET_LINK_VALIDITY - datetime.timedelta(days=3),
    ):
        reset_request.accept()
...
def test_mfa_reset_link_can_be_used_after_resend_after_link_expiration(settings, client):
...
def test_mfa_reset_cannot_be_resent_nor_used_after_request_expiration(settings, client):
    ...
    with mock.patch(
        "django.utils.timezone.now",
        return_value=timezone.now() - settings.MFA_RESET_REQUEST_VALIDITY - datetime.timedelta(days=3),
    ):
        reset_request = ResetRequestFactory(user=device.user)
        reset_request.accept()
