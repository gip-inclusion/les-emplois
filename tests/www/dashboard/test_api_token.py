...
def test_api_token_view_for_company_admin(client, mailoutbox):
    ...
    [email] = mailoutbox
    assert email.subject.endswith("Génération d’un nouveau token d’API sur La plateforme de l’inclusion")
...
def test_api_token_view_regenerate(client, mailoutbox):
    ...
    assert new_email.subject.endswith("Génération d’un nouveau token d’API sur La plateforme de l’inclusion")
