...
class TestEditJobSeekerInfo:
    ...
    def test_display_modal_when_form_changed(self, client, mocker, snapshot, born_in_france):
        ...
        # Subject
        assert "Vos données personnelles ont été mises à jour sur La plateforme de l’inclusion" in email.subject
        # Body
        ...
        assert len(email.bcc) == 0
        # Subject
        assert "Votre adresse e-mail a été mise à jour sur La plateforme de l’inclusion" in email.subject
        # Body
        assert job_seeker.get_full_name() in email.body
        assert job_application.to_company.name in email.body
    ...
    def test_name_is_required(self, client):
        ...
        job_application = JobApplicationFactory(
            sent_by_prescriber_alone=True, to_company=company, job_seeker__created_by=user
        )
        post_data = {
            "title": "M",
        ...
        assertContains(
            ...
            <div id="id_first_name_error" class="w-100">
