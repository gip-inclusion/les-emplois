...
def test_build_dora_orientation_payload(db):
    ...
    payload = build_dora_orientation_payload(
        ...
        documents_data={"gdpr_consent": True},
...
def test_build_dora_orientation_payload_uses_referent_phone_when_prescriber_has_no_phone(db):
    organization = PrescriberOrganizationFactory(siret="12345678901234")
    prescriber = PrescriberFactory(membership=True, membership__organization=organization, phone="")
    job_seeker = JobSeekerFactory()
    ...
    assert payload["emplois_data"]["prescriber_phone"] == "0506070809"
