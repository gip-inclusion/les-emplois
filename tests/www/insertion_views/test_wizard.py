...
def test_orientation_wizard_happy_path(client, snapshot, mailoutbox):
    ...
    details_url = reverse(
        ...
        kwargs={"orientation_id": orientation.pk},
    )
...
def test_orientation_wizard_happy_path_as_employer(client, mailoutbox):
...
def test_orientation_wizard_no_email_if_sender_is_referent(client, mailoutbox):
    organization = CompanyFactory()
    user = CompanyMembershipFactory(company=organization).user
    job_seeker = JobSeekerFactory()
    JobSeekerAssignmentFactory(professional=user, company=organization, job_seeker=job_seeker)
