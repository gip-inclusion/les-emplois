...
from itou.insertion.models import GenericReferenceItemKind, GenericReferenceItemSource
...
def test_detail_accessible_without_login(client, db):
...
def test_detail_basic_dora(client, snapshot):
...
def test_detail_basic_not_dora(client, snapshot):
    user = PrescriberFactory(membership=True)
    service = ServiceFactory(
        uid="test-service-uid",
        name="Mon service de test",
...
def test_detail_with_all_optional_fields(client, snapshot):
    user = PrescriberFactory(membership=True)
...
def test_detail_with_external_orientation_link(client, snapshot):
    user = PrescriberFactory(membership=True)
...
def test_detail_orientable_and_user_authenticated(client, snapshot):
    user = PrescriberFactory(membership=True)
    service = ServiceFactory(
        uid="test-orientable-uid",
        name="Service orientable",
...
def test_detail_orientable_and_user_not_authenticated(client, db):
    service = ServiceFactory(
        uid="test-orientable-uid",
        name="Service orientable",
        updated_on="2025-01-15",
    ...
    service_url = detail_url(service)
...
def test_detail_not_orientable(client, snapshot):
    user = PrescriberFactory(membership=True)
    service = ServiceFactory(
...
def test_detail_contact_section_hidden_without_contact_info(client, db):
    user = PrescriberFactory(membership=True)
    service = ServiceFactory(
...
def test_detail_contact_button_shown_when_authenticated(client, db):
    user = PrescriberFactory(membership=True)
    service = ServiceFactory(
...
def test_detail_contact_button_shown_when_public(client, db):
    service = ServiceFactory(
...
def test_detail_contact_login_link_shown_when_anonymous_and_not_public(client, db):
    service = ServiceFactory(
...
def test_detail_with_source_link(client, db):
    user = PrescriberFactory(membership=True)
...
def test_detail_without_source_link(client, db):
    user = PrescriberFactory(membership=True)
...
def test_detail_orientation_url_with_jwt_for_prescriber(client, is_authorized):
...
def test_detail_credential_documents(client, db, snapshot):
    service = ServiceFactory(
        uid="test-creds-uid",
        updated_on="2025-01-15",
...
def test_format_categories_no_thematics(client, db):
    service = ServiceFactory(
...
def test_format_categories_single_thematic(client, db):
    thematic = GenericReferenceItemFactory(
        kind=GenericReferenceItemKind.THEMATIC,
...
def test_format_categories_multiple_categories(client, db):
...
def test_professionals_has_autre_true_when_autre_mode_selected(client, db):
...
def test_professionals_has_autre_false_without_autre_mode(client, db):
    mode = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_PROFESSIONAL,
        value="telephonique",
...
def test_beneficiaries_has_autre_true_when_autre_mode_selected(client, db):
    mode = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
...
def test_beneficiaries_has_autre_false_without_autre_mode(client, db):
    mode = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_BENEFICIARY,
        value="en-presentiel",
...
def test_autre_mode_label_not_rendered_in_list(client, db):
...
def test_other_field_shown_when_autre_mode_selected(client, db):
    mode_autre = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_PROFESSIONAL,
        value="autre",
...
def test_other_field_not_shown_without_autre_mode(client, db):
    mode_phone = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_PROFESSIONAL,
        value="telephonique",
...
def test_beneficiaries_autre_mode_label_not_rendered_in_list(client, db):
    mode_autre = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_BENEFICIARY,
        value="autre",
...
def test_beneficiaries_other_field_not_shown_without_autre_mode(client, db):
    mode_presentiel = GenericReferenceItemFactory(
        source=GenericReferenceItemSource.DORA,
        kind=GenericReferenceItemKind.MOBILIZATION_BENEFICIARY,
        value="en-presentiel",
