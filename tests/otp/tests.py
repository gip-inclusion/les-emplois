...
def test_mfa_reset_mail_sent_when_request_accepted(settings, mailoutbox, django_capture_on_commit_callbacks):
...
def test_mfa_reset_resend_link(settings, mailoutbox, django_capture_on_commit_callbacks):
    settings.REQUIRE_OTP_FOR_STAFF = True
    device = ItouTOTPDeviceFactory(name="1")
    reset_request = ResetRequestFactory(user=device.user)
...
def test_mfa_reset_lowest_credibility():
...
def test_mfa_reset_good_credibility():
    user = ItouStaffFactory()
