...
from tests.audit_trail.factories import AuditTrailEventType, AuditTrailFactory
from tests.otp.factories import ItouTOTPDeviceFactory, ResetRequestFactory
from tests.users.factories import ItouStaffFactory


def test_mfa_reset_mail_sent_when_request_accepted(mailoutbox, django_capture_on_commit_callbacks):
...
def test_mfa_reset_resend_link(mailoutbox, django_capture_on_commit_callbacks):
    device = ItouTOTPDeviceFactory(name="1")
    reset_request = ResetRequestFactory(user=device.user)
...
SUNDAY_NIGHT = datetime.datetime(2026, 6, 21, 2, tzinfo=datetime.UTC)
FRIDAY_MORNING = datetime.datetime(2026, 8, 28, 12, tzinfo=datetime.UTC)


def test_mfa_reset_worst_credibility():
    user = ItouStaffFactory()
    with freeze_time(SUNDAY_NIGHT):
        reset_request = ResetRequestFactory(user=user)

    # Built a very suspicious audit trail:
    with freeze_time(timezone.now() - datetime.timedelta(days=8)):
        AuditTrailFactory(user=user, event_type=AuditTrailEventType.LOG_IN, browser_id="Browser 1", ip="9.9.9.9")
    AuditTrailFactory(
        user=user,
        event_type=AuditTrailEventType.MFA_RESET_REQUEST,
        browser_id="Browser 2",
        ip="1.2.3.4",
        data={"itou_mfa_reset_request_pk": reset_request.pk},
    )

    assert reset_request.credibility() == 0
    assert reset_request.is_suspicious


def test_mfa_reset_best_credibility():
    user = ItouStaffFactory()
    with freeze_time(FRIDAY_MORNING):
        reset_request = ResetRequestFactory(user=user)

    # A clean audit trail:
    with freeze_time(timezone.now() - datetime.timedelta(days=8)):
        AuditTrailFactory(user=user, event_type=AuditTrailEventType.LOG_IN, browser_id="Browser 1")
    AuditTrailFactory(
        user=user,
        event_type=AuditTrailEventType.MFA_RESET_REQUEST,
    ...
    assert reset_request.credibility() == 1
    assert not reset_request.is_suspicious


@pytest.mark.parametrize("timestamp", [SUNDAY_NIGHT, FRIDAY_MORNING])
def test_mfa_reset_different_browser_and_different_ip_is_always_suspicious(timestamp):
    user = ItouStaffFactory()
...
def test_mfa_reset_close_expired_command_keeps_fresh_requests():
    reset_request = ResetRequestFactory()
    call_command("close_expired_mfa_reset_requests")
    reset_request.refresh_from_db()
...
def test_mfa_reset_close_expired_command_grace_for_freshly_accepted_requests():
    """Let's say we have and old request that should be closed by the
    ...
    should_expire_at = created_at + settings.MFA_RESET_REQUEST_VALIDITY
    right_before_expiration = should_expire_at - datetime.timedelta(hours=1)
    right_after_expiration = should_expire_at + datetime.timedelta(hours=1)
    long_after_expiration = should_expire_at + settings.MFA_RESET_LINK_VALIDITY + datetime.timedelta(hours=1)
...
def test_mfa_reset_close_expired_command_grace_for_freshly_resent_requests():
    ...
    """
    created_at = datetime.datetime(1984, 4, 4, tzinfo=datetime.UTC)
    should_expires_at = created_at + settings.MFA_RESET_REQUEST_VALIDITY
    ...
    long_after_expiration = should_expires_at + settings.MFA_RESET_LINK_VALIDITY + datetime.timedelta(hours=1)
