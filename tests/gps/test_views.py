...
class TestJoinGroup:
    ...
    def test_view_with_org(self, client, snapshot, user_factory):
        ...
        assertRedirects(response, reverse("gps:join_group_from_collegue"), fetch_redirect_response=False)
...
class TestBeneficiariesAutocomplete:
    ...
    def test_autocomplete(self, client):
        ...
        collegue_1 = PrescriberMembershipFactory(organization=organization_1).user
        organization_2 = PrescriberMembershipFactory(user=prescriber).organization
        collegue_2 = PrescriberMembershipFactory(organization=organization_2).user
        ...
        FollowUpGroupFactory(beneficiary=second_beneficiary, memberships=2, memberships__member=collegue_1)
        FollowUpGroupFactory(beneficiary=third_beneficiary, memberships=3, memberships__member=collegue_2)
...
class TestJoinGroupFromCollegue:
    URL = reverse("gps:join_group_from_collegue")
    ...
    def test_view(self, client, snapshot):
        ...
        collegue = CompanyMembershipFactory(company=company).user
        ...
        collegue_job_seeker = JobSeekerFactory()
        group = FollowUpGroupFactory(beneficiary=collegue_job_seeker, memberships=1, memberships__member=collegue)
        response = client.post(self.URL, data={"user": collegue_job_seeker.pk})
        assertRedirects(response, reverse("gps:group_list"))
        assert_new_beneficiary_toast(response, collegue_job_seeker)
    ...
    def test_toasts(self, client, can_view_personal_info):
        ...
        collegue = PrescriberMembershipFactory(organization=organization).user
        client.force_login(user)

        collegue_job_seeker = JobSeekerFactory()
        FollowUpGroupFactory(beneficiary=collegue_job_seeker, memberships=1, memberships__member=collegue)
        response = client.post(self.URL, data={"user": collegue_job_seeker.pk}, follow=True)
        assert_new_beneficiary_toast(response, collegue_job_seeker, can_view_personal_info)

        response = client.post(self.URL, data={"user": collegue_job_seeker.pk}, follow=True)
        assert_already_followed_beneficiary_toast(response, collegue_job_seeker, can_view_personal_info)
