...
def test_import_advisor_information(settings, caplog, mocker):
    ...
    # each group only has one membership
    assert FollowUpGroupMembership.objects.count() == 5
    ...
    # Job seeker 3:
    membership_3 = FollowUpGroupMembership.objects.get(follow_up_group__beneficiary=job_seeker_3)
