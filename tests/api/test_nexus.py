...
class TestUserAPI(NexusApiTestMixin):
    ...
    def assert_user_equals(self, user, source, user_data):
        assert user.pk == f"{source}--{user_data['id']}"
...
class TestSyncCompletedAPI(NexusApiTestMixin):
    ...
    def test_invalid_payloads(self):
        ...
        NexusRessourceSyncStatusFactory(service=Service.EMPLOIS, new_start_at=start_at)
        response = api_client.post(
            self.url, data={"started_at": start_at.isoformat()}, content_type="application/json"
        )
        assert response.status_code == 403

        # new_start_at dosen't match
        NexusRessourceSyncStatusFactory(service=Service.DORA, new_start_at=start_at + datetime.timedelta(seconds=1))
        response = api_client.post(
            self.url, data={"started_at": start_at.isoformat()}, content_type="application/json"
        )
        assert response.status_code == 403
