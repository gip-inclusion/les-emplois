...
class TestCertifyCriteriaWithFranceTravail:
    ...
    def test_queue_task(self, respx_mock, job_seeker_kwargs, endpoint):
        ...
        criterion = eligibility_diagnosis.selected_administrative_criteria.get()
        rechercher_usager_url = urljoin(settings.API_ESD["BASE_URL"], endpoint)
        ...
        certify_rqth_url = urljoin(settings.API_ESD["BASE_URL"], PE_Endpoints.RQTH)
    ...
    def test_not_certified(self, respx_mock):
        ...
        rechercher_usager_nir_endpoint = urljoin(
            settings.API_ESD["BASE_URL"], PE_Endpoints.RECHERCHER_USAGER_DATE_NAISSANCE_NIR
        )
        respx_mock.post(rechercher_usager_nir_endpoint).respond(
            200,
        ...
        assert [call.request.url for call in respx_mock.calls] == [
            "https://auth.fr/connexion/oauth2/access_token?realm=%2Fagent",
            rechercher_usager_nir_endpoint,
            certify_rqth_url,
        ]
    ...
    def test_rechercher_usager_issues(self, caplog, certification_period, response_kind, respx_mock, user_found):
        ...
        criterion = eligibility_diagnosis.selected_administrative_criteria.get()
        rechercher_usager_nir_endpoint = PE_Endpoints.RECHERCHER_USAGER_DATE_NAISSANCE_NIR
        json_response = API_POLE_EMPLOI_RESPONSES[rechercher_usager_nir_endpoint][response_kind]
        rechercher_usager_nir_url = urljoin(settings.API_ESD["BASE_URL"], rechercher_usager_nir_endpoint)
        respx_mock.post(rechercher_usager_nir_url).respond(200, json=json_response)

        async_certify_criterion_with_france_travail.call_local(criterion._meta.model_name, criterion.pk)

        assert [call.request.url for call in respx_mock.calls] == [
            "https://auth.fr/connexion/oauth2/access_token?realm=%2Fagent",
            rechercher_usager_nir_url,
        ]
        criterion.refresh_from_db()
    ...
    def test_bad_response(self, respx_mock):
        ...
        rechercher_usager_nir_endpoint = PE_Endpoints.RECHERCHER_USAGER_DATE_NAISSANCE_NIR
        rechercher_usager_nir_url = urljoin(settings.API_ESD["BASE_URL"], rechercher_usager_nir_endpoint)
        json_response = API_POLE_EMPLOI_RESPONSES[rechercher_usager_nir_endpoint][
            ApiPoleEmploiResponseKind.INTERNAL_SERVER_ERROR
        ]
        respx_mock.post(rechercher_usager_nir_url).respond(500, json=json_response)
    ...
    def test_rate_limit(self, respx_mock):
        ...
        rechercher_usager_nir_endpoint = PE_Endpoints.RECHERCHER_USAGER_DATE_NAISSANCE_NIR
        rechercher_usager_nir_url = urljoin(settings.API_ESD["BASE_URL"], rechercher_usager_nir_endpoint)
        respx_mock.post(rechercher_usager_nir_url).respond(429, headers={"Retry-After": "42"})
