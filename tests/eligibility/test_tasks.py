...
class TestCertifyCriteriaWithFranceTravail:
    ...
    @pytest.mark.parametrize(
        "response_kind,certification_period",
        [
            pytest.param(ApiPoleEmploiResponseKind.MULTIPLE_USERS_RETURNED, None, id="multiple-users"),
            pytest.param(ApiPoleEmploiResponseKind.NOT_FOUND, None, id="no-user"),
            pytest.param(ApiPoleEmploiResponseKind.NOT_CERTIFIED, InclusiveDateRange(empty=True), id="not-certified"),
        ],
    )
    def test_rechercher_usager_issues(self, caplog, certification_period, response_kind, respx_mock):
        eligibility_diagnosis = IAEEligibilityDiagnosisFactory(
            certifiable=True,
            criteria_kinds=AdministrativeCriteriaKind.certifiable_by_api_france_travail(),
        )
